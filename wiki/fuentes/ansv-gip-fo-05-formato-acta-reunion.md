---
title: "Formato Acta de Reunión (ANSV-GIP-FO-05) — Anexo 5 del bundle PR-06"
type: fuente
tags: [ansv, gip, formato, acta-de-reunion, asistencia-tecnica, sistema-de-gestion-de-calidad, plantilla]
fuente_pdf: "Anexo 5. Instrucciones para el registro de acta de reunión.pdf"
status: ingerido
last_updated: 2026-09-21
---

## Referencia / origen del documento

- Título: "Proceso Gestión Integral de Procesos — Formato Acta de Reunión" (p. 1).
- Código: **ANSV-GIP-FO-05**. Versión: 03. Fecha: 13/08/2025 (p. 1).
- Documento de 3 páginas.
- Metadato del archivo: `/Author: Claudia Ruiz`, `/Creator: Microsoft® Word LTSC` (p. 1, sin sección de "Aprobación" firmada dentro del propio PDF). `[INCIERTO: no hay una sección de elaboró/revisó/aprobó como en el PR-06/PR-07/PR-08/CA-02; no se puede determinar si Claudia Ruiz es la autora del contenido o solo de la última edición del archivo.]`

`[INCIERTO: a diferencia del PR-06, PR-07, PR-08 y CA-02 — cuyo código empieza con "ANSV-CPP-" (proceso de Coordinación y Articulación para la Política Pública) —, este documento pertenece a un proceso distinto: "ANSV-GIP-" (Gestión Integral de Procesos). Es decir, no es un procedimiento propio de la DCI sino un **formato transversal de toda la ANSV**, reutilizado por la DCI para sus actas de asistencia técnica. Su versión (03) y fecha (13/08/2025) también son anteriores y de otro linaje documental que el PR-06/PR-07/PR-08/CA-02 (todos versión 00, diciembre 2025-enero 2026).]`

## Qué es y para qué sirve

No es un procedimiento ni una caracterización de proceso, sino una **plantilla con instrucciones de diligenciamiento** para el formato de acta de reunión que usa toda la ANSV, incluida la DCI. El PDF combina, campo por campo, el rótulo del campo con la instrucción de cómo diligenciarlo (en cursiva/itálica en el original) — no contiene una reunión real ya diligenciada.

Nota de actualización (p. 1): "Descargar el formato de Acta de Reunión actualizado en el aplicativo SIG MIPG. Consultar siempre en el siguiente link: SIG-MIPG ANSV" — es decir, el propio documento advierte que la versión vigente puede no ser esta y remite a un aplicativo interno no disponible en este repositorio.

## Contenido clave (estructura del formato, p. 1-3)

- **ACTA No.**: instrucción de numerar consecutivamente por tema, empezando en 1 cada año; "cuando no se requiera llevar consecutivo, indicar 'No aplica'" (p. 1).
- **TEMA**: instrucción explícita y directamente relevante para el trabajo en la ANSV (p. 1, cita textual): "En el registro de la actividad se debe especificar la temática de la asistencia técnica, asegurando su alineación con la estrategia o documento del PNSV correspondiente. La actividad debe nombrarse siempre como 'Asistencia Técnica', indicando su enfoque". Ejemplos dados en el propio formato:
  - "Asistencia Técnica – PLSV de Cartagena"
  - "Asistencia Técnica – Acompañamiento / Capacitación / Taller"
  - "Asistencia Técnica a Organismos de Tránsito"
  - "Asistencia Técnica – [otra temática que aplique]"
  
  El formato exige además que "la temática registrada debe ser coherente con el objetivo de la actividad, los lineamientos del PNSV y los indicadores institucionales" (p. 1).
- **FECHA, HORA INICIO, HORA FIN, LUGAR** (p. 1): campos estándar; "Lugar" pide ciudad/municipio y departamento (si es distinto de Bogotá) y el medio virtual si aplica.
- **ASISTENTES E INVITADOS** (p. 1): remite a un formato distinto, "Ver registro de asistencia (ANSV-GIP-FO-06)" — **no disponible en este repositorio**.
- **ORDEN DEL DÍA** y **DESARROLLO DE LA REUNIÓN** (p. 1-2): el desarrollo "debe reflejar las actividades realizadas durante la asistencia técnica, los principales aportes, orientaciones brindadas y conclusiones" (p. 2).
- **COMPROMISOS ADQUIRIDOS** (p. 2): tabla con columnas Compromiso / Entregable / Responsable / Fecha. Instrucción explícita sobre trazabilidad (p. 2, cita textual): "Cada compromiso debe contar con un entregable claramente definido; en caso de no aplicar, deberá indicarse expresamente 'No aplica'. No obstante, siempre que haya lugar, se deberá establecer fecha de cumplimiento y responsable, con el fin de garantizar el seguimiento, control y trazabilidad de las acciones acordadas."
- **CONVOCATORIA** (p. 2): campo para indicar la fecha de la próxima reunión.
- **Firma del responsable de la reunión** (nombre y cargo) y **Anexos** (p. 3): "liste los anexos que se manejaron en el desarrollo de la reunión, en caso de no contar con anexos registrar 'No aplica'".

## Relación con las demás fuentes de este repositorio

- [ansv-pr-06-procedimiento-asistencia-tecnica](ansv-pr-06-procedimiento-asistencia-tecnica.md): el PR-06 usa "acta de reunión" como registro en varias de sus 25 actividades. Este Anexo 5 es la plantilla concreta de ese registro, y confirma con instrucción explícita que toda actividad de asistencia técnica debe registrarse bajo el rótulo "Asistencia Técnica" en el campo TEMA — dato operativo que el propio PR-06 no detalla.
- [ansv-pr-07-estrategias-interinstitucionales](ansv-pr-07-estrategias-interinstitucionales.md) y [ansv-pr-08-articulacion-instancias-territoriales](ansv-pr-08-articulacion-instancias-territoriales.md): ambos usan "acta de reunión" como registro en varias actividades (PR-07: actividades 4, 7, 8, 9, 10; PR-08: 8 de sus 14 actividades) — todas presumiblemente diligenciadas con este mismo formato ANSV-GIP-FO-05, aunque ninguna de las dos fuentes lo cita por su código exacto.
- [ansv-ca-02-caracterizacion-proceso](ansv-ca-02-caracterizacion-proceso.md): "actas de reunión" figura entre las salidas/productos mencionados del ciclo Hacer.
- [dci-lineamientos-cargue-evidencias](dci-lineamientos-cargue-evidencias.md) — refuerza y hace más estricta la misma regla de nomenclatura del campo TEMA de este formato, agregando la prohibición explícita de términos alternativos y el requisito de nomenclatura del archivo PDF.
- Formato relacionado explícitamente citado pero **no disponible en este repositorio**: ANSV-GIP-FO-06 (registro de asistencia).

## Relevancia para el trabajo en la ANSV

`[Interpretación propia, no una afirmación de la fuente]`: este anexo es la evidencia documental más concreta, hasta ahora, de cómo la ANSV **exige nombrar y trazar** cada intervención de asistencia técnica: el requisito de que el campo TEMA siempre empiece con "Asistencia Técnica" no es un detalle menor para una propuesta de protocolo — es el mecanismo por el cual, en teoría, toda asistencia técnica queda identificable y auditable en el repositorio documental de la DCI. Para una propuesta de protocolo dirigida a un municipio u organismo de tránsito, esto fija un estándar de nomenclatura y de registro de compromisos (entregable + responsable + fecha) que cualquier protocolo nuevo debería respetar para ser compatible con el SGC vigente de la ANSV.

## Conceptos y entidades mencionados

**Entidades / dependencias:**
- Agencia Nacional de Seguridad Vial (ANSV) (p. 1)
- Sistema Integrado de Gestión / MIPG (aplicativo SIG MIPG) (p. 1)

**Conceptos / instrumentos:**
- Plan Nacional de Seguridad Vial (PNSV) (p. 1)
- Plan Local de Seguridad Vial (PLSV) (p. 1, en el ejemplo "Asistencia Técnica – PLSV de Cartagena")
- Asistencia Técnica, Acompañamiento, Capacitación, Taller (p. 1, como enfoques posibles del campo TEMA)
- Registro de asistencia (ANSV-GIP-FO-06) — formato relacionado no disponible en este repositorio (p. 1)

## Incertidumbres

- `[INCIERTO: no hay sección de elaboró/revisó/aprobó dentro del propio PDF — ver "Referencia / origen del documento".]`
- `[INCIERTO: este formato pertenece al proceso "Gestión Integral de Procesos" (GIP), no al proceso "Coordinación y Articulación..." (CPP) del PR-06/PR-07/PR-08/CA-02 — es un formato transversal reutilizado, no propio de la DCI. No se puede confirmar desde esta fuente si existe una versión de acta de reunión específica para la DCI.]`
- No se puede verificar el contenido del formato ANSV-GIP-FO-06 (registro de asistencia) porque no está en este repositorio.

## Ver también

- [conceptos/asistencia-tecnica-bundle-pr06](../conceptos/asistencia-tecnica-bundle-pr06.md)
- [entidades/dci-direccion-coordinacion-interinstitucional](../entidades/dci-direccion-coordinacion-interinstitucional.md)
